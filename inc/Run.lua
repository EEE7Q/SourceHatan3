...
print('\n\27[1;31m￤ Conect is Failed !\n￤ -› حدث خطأ في الاتصال بالسيرفر , يرجى مراسلة مطور السورس ليتمكن من حل المشكـلة في اسرع وقت ممكن . !')
create_config(Token)
end
GetUser.result.username = GetUser.result.username or GetUser.result.first_name
print('\n\27[1;36m￤-› تم ادخال ايدي المطور بنجاح , سـوف يتم تشغيل السورس الآن .\n￤Success Save USERID : \27[0;32m['..SUDO_USER..']\n\27[0;39;49m')
max = Token:match("(%d+)")
...
local Text = "-› اهلاً عزيزي [المطور الاساسي](tg://user?id="..SUDO_USER..") \n- شكرا لاستخدامك سورس هتان .\n- - ارسل /start الآن .\n- لا اضهار الاوامر المجهزه بالكيبورد .\n\n⋆"
...
SUDO_USER = redis:hgetall(max..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(max..':WELCOME_BOT'),[[-› مـرحبآ انا بوت اسمي ]]..redis:get(max..':NameBot:')..[[ 
...
redis:hset(max..'username:'..tonumber(mmd),'username',msg.text)
	send_msg(msg.chat_id_,"-› تم عزيزي تغيير المّـطور الاساسي بنجاح الان ارسل reload ...⋆")
	end
...
send_msg(msg.chat_id_,"-› تم تثبيت الايـدي , الآن قم بأرسـال يُـوزر المّطور @UserName ...")
	end
...
redis:setex('setid'..msg.sender_user_id_,120,true)
	...
	sendMsg(msg.chat_id_,msg.id_,'-- {* تــم أعـادة تشغيل البوت  *} --.\n\n- { Bot is Reloaded » }👍🏿',nil,function(arg,data)
	dofile("./inc/Run.lua")
	print("Reload ~ ./inc/Run.lua")
	end)
	return false
...
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	send_msg(SUDO_ID,"-› قام شخص بطرد البوت من المجموعة ! : \n- الايدي ⇽`-100"..data.channel_.id_.."`\n- المجّـموعة ⇽"..Flter_Markdown(NameGroup).."\n\n— تم مسح كل بيانات المجّــموعة . —")
	...
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'- تم رفع نسخة احتياطية .\n- حالياً عدد مجموعاتك العدد ⇽'..redis:scard(max..'group:ids')..'*\n✓')
	end
