...
message = '-› قائمة المطوريـن :\n\n`- المطور الاساسي`**\n'..SUDO_USER..' » ❪' ..SUDO_ID.. '❫\n⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆\n'
if #list==0 then  message = message.."- لا يوجد مطورين حالياً ."
else
...
if #monsha == 0 then 
message = message .."Not Creator-›  لا يوجـد مالكين .\n"
else
...
if #list == 0 then  
message = message.."Not Director -› لا يوجـد مدراء .\n" 
else
for k,v in pairs(list) do
local info = redis:hgetall(max..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
...
if #list==0 then  return  "-› لا يوجـد ادمنيـه ." end
message = '-› قائمة الادمنيـه :\n\n'
for k,v in pairs(list) do
local info = redis:hgetall(max..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.. '~⪼ '..(info.username or '')..' » ❪`' ..v.. '`❫ \n'
...
if #list == 0 then return "-› لا يوجـد مميزين ." end
...
function vorr(msg) 
local message = '-› قائمة الثـور :\n\n'
local monsha = redis:smembers(max..':vor:'..msg.chat_id_)
if #monsha == 0 then 
message = message .."-› لا يوجـد احد مرفوع ."
else
for k,v in pairs(monsha) do
local info = redis:hgetall(max..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
...
if #list==0 then return "-› لا يوجـد اعضاء مكتومين ." end
...
if #list==0 then return " -› لا يوجـد اعضاء محظورين ." end
...
if #list==0 then return  "-› لا يوجـد اعضاء محظورين عام ." end
...
local list = redis:smembers(max..':Filter_Word:'..msg.chat_id_)
if #list == 0 then return "-› لاتوجـد كلمات ممنوعه ." end
filterlist = '-› قائمة الكلمات الممنـوعه :\n'    
for k,v in pairs(list) do
...
return  "- الكلمة *{"..word.."}* هي بالتأكيد من قائمة المنع ." 
else
redis:sadd(max..':Filter_Word:'..msg.chat_id_,word) 
return  "- الكلمة *{"..word.."}* تم اضافتها الى قائمة المنع ."
end
end
...
return  "- الكلمة *{"..word.."}* هي بالتأكيد مسموح بها ." 
end
end
...
Save_Data:close()
sendDocument(msg.chat_id_,msg.id_,"./inc/"..Bot_User..".json","-› ملف النسخه الاحتياطـيه ...\n- المجموعات ⇽ { "..#All_Groups_ID.." }\n- للبوت ⇽"..Bot_User.."\n- التاريخ ⇽ "..os.date("%Y/%m/%d").."\n",dl_cb,nil)
end

function chat_list(msg)
local list = redis:smembers(max..'group:ids')
message = '-› قائمة المجّـموعات :\n\n'
for k,v in pairs(list) do 
...
all_groups = '-› قائمة المجّـموعات :<br><br>'
for k,v in pairs(list) do 
local info = redis:get(max..'group:name'..v)
if info then
...
sendMsg(msg.chat_id_,1,'-› عذراً لديك الكثير من المجموعات\n- سوف ارسل لك ملف فيها قائمة المجموعات انتظر لحظه ...')
...
file:close()
return sendDocument(msg.chat_id_,msg.id_,'./inc/All_Groups.html','-› قامة المجموعات بالكامله .\n- يحتوي ('..#list..') مجّـموعة\n- افتح الملف في عارض HTML او بالمتصفح',dl_cb,nil)
else 
...
if NumAdmin == 0 then 
return sendMsg(msg.chat_id_,msg.id_,"-› لا يوجـد ادمنيه ليتم رفعهم !\n")
else
return sendMsg(msg.chat_id_,msg.id_,"-› تم رفع { *"..NumAdmin.."* } من الادمنيه في البوت .\n✓️")
end
end,100)
end
...
if not msg.SudoUser and not service then return '-› انت لست المّـطور .' end
if msg.is_post_ then return "عذراً هذا بوت حمـاية مجموعات وليس للقنوات ." end
if msg.type ~= "channel" then return '-› لا يمكـنك تفعيل البوت بالمجموعة العـاديه ، الحل اعطني الصلاحيات بأكملها ليتم تفعيلي او كلم المطور .' end
...
return "- اشترك بالقنـاة اولاً["..UserChaneel.."] \nثم ارجع ارسل تفعيل ."
end
else
return "- اشترك بالقنـاة اولاً["..UserChaneel.."] \nثم ارجع ارسل تفعيل ."
end
...
return sendMsg(msg.chat_id_,msg.id_,'-› لا يمكـنك تفعيل البوت بالمجموعة يجب ان يكـون اكـثر من 【'..GroupUsers..'】 عضو .')
else 
GetChatMember(msg.chat_id_,our_id,function(arg,data)
if data.status_.ID == "ChatMemberStatusMember" then
return sendMsg(msg.chat_id_,msg.id_,'-› عذراً البوت ليس مشرف بالمجموعة\nيجي ترقيته لتتمكن من تفعـيل البوت .')
else
local lock_service = redis:get(max..'lock_service')
if lock_service then 
sendMsg(msg.chat_id_,msg.id_,'-› تم تفعيل البوت بالمجّـموعة\n- وتم رفع كل المشرفين .\n⋆')
else
sendMsg(msg.chat_id_,msg.id_,'-› تم تفعيل المجّـموعة .\n✓️')
end
GetChannelAdministrators(msg.chat_id_,function(arg,data)
for k,v in pairs(data.members_) do
...
USERNAME_T = '-› اليُــوزر ⇽❪ @['..datai.username_..'] ❫\n'
else 
USERNAME_T = ''
end
return send_msg(SUDO_ID,'-› تم تفعـيل البـوت ..\n⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆\n-› معلومات المجّـموعة :\n'
..'- اسم المجّـموعة ⇽❪ ['..NameGroup..']('..Gp_Link..') ❫\n'
..'- ايدي المجّـموعة ⇽❪ '..msg.chat_id_..' ❫\n'
..'- عدد اعضاء المجّـموعة ⇽❪ *'..Groupcount..'* ❫ \n⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆\n-› معلومات الشخص اللي ضافني :\n'
..'- الاسـم ⇽ ❪ ['..FlterName(datai.first_name_..' '..(datai.last_name_ or ""),23)..'](tg://user?id='..msg.sender_user_id_..') ❫\n'
..USERNAME_T..'\n- تاريخ التفعيل ⇽ ❪* '..os.date("%Y/%m/%d")
..' *❫\n- السـاعه ⇽ ❪* '..os.date("%I:%M%p")..' *❫')
end,nil)
end
end)
end,Get_Director) 
...
function modrem(msg)
if not msg.SudoUser then return '-› انت لست المّـطور .'end
if not redis:get(max..'group:add'..msg.chat_id_) then return '-› تم تعطيل المجّـموعة سابقاً .' end  
rem_data_group(msg.chat_id_)
return '-› تم تعطيل المجّـموعة .'
end
...
if cmd == "voo" then
if UserID == our_id then 
return sendMsg(ChatID,MsgID,"-› عذراً لا يمكن رفع نفسي .") 
elseif data.type_.ID == "UserTypeBot" then
return sendMsg(ChatID,MsgID,"- لا يمكن رفع بوت في البوت .") 
...
return SendMention(ChatID,UserID,MsgID,'- العضو ⇽ ❪ '..USERNAME..' ❫\n- الايدي ⇽ ❪ '..UserID..' ❫\n- انهُ بالتأكيد دجاجـه .\n✓️',17,USERCAR) 
end
redis:sadd(max..':zmm:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'- العضو ⇽ ❪ '..USERNAME..' ❫\n- الايدي ⇽ ❪ '..UserID..' ❫\n- تم ترقيـته ليصبح دجاجـه .\n✓️',17,USERCAR) 
end
...
return SendMention(ChatID,UserID,MsgID,'- العضو ⇽ ❪ '..USERNAME..' ❫\n- الايدي ⇽ ❪ '..UserID..' ❫\n- انهُ بالتأكيد ذبانـه .\n✓️',17,USERCAR) 
end
redis:sadd(max..':tta:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'- العضو ⇽ ❪ '..USERNAME..' ❫\n- الايدي ⇽ ❪ '..UserID..' ❫\n- تم ترقيـته ليصبح ذبانـه .\n✓️',17,USERCAR) 
end
...
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"-› لا يمكنك تقييد المّـطور الاساسي .") 
elseif redis:sismember(max..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"-› لا يمكنك تقييد المّـطور .") 
elseif redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
...
if cmd == "iduser" then
return SendMention(ChatID,UserID,MsgID,"-› تفضل الايدي واليُـوزر :\n\n "..USERNAME.." ⟷ { "..UserID.." }",37,USERCAR)
end
if cmd == "whois" then
...
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"-› لا يمكنك حظر المّـطور الاساسي .") 
elseif redis:sismember(max..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"-› لا يمكنك حظر المّـطور .") 
elseif redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
...
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"-› لا يمكنك كتم المّـطور الاساسي .") 
elseif redis:sismember(max..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"-› لا يمكنك كتم المّـطور .") 
elseif redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
...
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"-› لا يمكنك طرد المّـطور الاساسي .") 
elseif redis:sismember(max..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"-› لا يمكنك طرد المّـطور .") 
elseif redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
...
elseif data.type_.ID == "UserTypeBot" then
return sendMsg(ChatID,MsgID,"-› عذراً لا يمكن رفع البوت في البوت .") 
end
...
return sendMsg(ChatID,MsgID,"-› عذراً هذا العضو ليس موجود ضمن المجموعات \n!")
end 
end
...
elseif data.type_.ID == "UserTypeBot" then
return sendMsg(ChatID,MsgID,"-› عذراً لا يمكنك رفع البوت في البوت .") 
elseif data.type_.ID == "ChatTypeChannel" then 
return sendMsg(ChatID,MsgID,"-› عذراً لا يمكنك رفع قناة في البـوت .") 
end
if redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'- العضو ⇽ ❪ '..UserName..' ❫\n- الايدي ⇽ ❪ `'..UserID..'` ❫\n- انهُ بالتأكيد المالك .\n✓️') 
end
redis:hset(max..'username:'..UserID, 'username', UserName)
redis:sadd(max..':MONSHA_BOT:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'- العضو ⇽ ❪ '..UserName..' ❫\n- الايدي ⇽ ❪ `'..UserID..'` ❫\n- تم ترقيـته ليصبح المالك .\n✓️')
end

if cmd == "remmnsha" then
if not redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'- العضو ⇽ ❪ '..UserName..' ❫\n- الايدي ⇽ ❪ `'..UserID..'` ❫\n- انهُ بالتأكيد ليس المالك .\n✓️') 
end
redis:srem(max..':MONSHA_BOT:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'- العضو ⇽ ❪ '..UserName..' ❫\n- الايدي ⇽ ❪ `'..UserID..'` ❫\n- تم تنريله من المالك .\n✓️') 
end
...
if cmd == "iduser" then
return sendMsg(ChatID,MsgID,"-› تفضل الايدي واليُـوزر :\n\n "..UserName.." ⟷ ( `"..UserID.."` )")
end
...
if UserID == our_id then 
return sendMsg(ChatID,MsgID,"-› عذراً لا يمكنني حظر نفسي .") 
elseif data.type_.ID == "ChannelChatInfo" then 
return sendMsg(ChatID,MsgID,"-› عذراً لا يمكن حظر قناة .") 
end
...
return sendMsg(ChatID,MsgID,"! لا يوجد عضـو بهذا المـعرف !")
end 
...
sendMsg(ChatID,MsgID,"- لا يوجد عضو !") 
return false
end
local UserID = data.id_
local Resolv = ResolveUser(data)
