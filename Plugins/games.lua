...
--[[
الالعاب لسـورس هتان
تم تعديل الملف واضافه الالعاب من قبل يُوسف @EEE7Q
--]]
local function games(msg,MsgText)
if msg.type ~= "pv" then
if MsgText[1] == "تفعيل" and MsgText[2] == "الالعاب" or MsgText[2] == "اللعبه" or MsgText[2] == "اللعبة" then
if not msg.Admin then return "-› هذا الامر يخص :  - (الادمن، المدير، المالك، المطور) فقط ." end
...
return "-› هذا الامر يخص :"..msg.TheRankCmd.."\nالالعاب بالتأكيد معطله\n✓" 
else
...
sendMsg(msg.chat_id_,msg.id_,'ارسل عدد الرسائل الذي تريده*')
end
...
sendMsg(msg.chat_id_,msg.id_,'ارسل عدد النقاط الذي تريده*')
end
...
katu = {
'تخفى','السوء','مقداره','فار','شقره','النخله','دواهي','تشكيلي','حامي','فرسانها','الحبل','طيعه','الشبل','الطريق','الخلق','دواء','شهر','سعادتك','القرد','عماها',
};
name = katu[math.random(#katu)]
redis:set(max..':Set_Amthlh:'..msg.chat_id_,name)
name = string.gsub(name,'تخفى','الحقيقة كالنورلا ___')
name = string.gsub(name,'السوء','يا حافر حفيرت___لا تعمقها تقع فيها')
name = string.gsub(name,'مقداره','من خرج من داره قلّ ___')
name = string.gsub(name,'فار','إن غاب القط العب يا ___')
name = string.gsub(name,'شقره','مع الخيل يا___')
name = string.gsub(name,'النخله','الطول طول___والعقل عقل الصخلة')
name = string.gsub(name,'دواهي','تحت السواهي ___')
name = string.gsub(name,'تشكيلي','لا ___ لأبكيلك')
name = string.gsub(name,'حامي','طق الحديد وهو ____')
name = string.gsub(name,'فرسانها','الرجال تعرف الرجال والخيل تعرف ___')
name = string.gsub(name,'الحبل','اللي تلدغة الحية يخاف من جرة___')
name = string.gsub(name,'طيعه','إذا ما طاعك الزمن ___')
name = string.gsub(name,'الشبل','هذا__من ذاك الأسد')
name = string.gsub(name,'الطريق','الرفيق قبل __')
name = string.gsub(name,'الخلق','حسن __ أحد مراكب الحياة')
name = string.gsub(name,'دواء','الكذب داء والصدق ___')
name = string.gsub(name,'شهر','امشي__ولا تعبر نهر')
name = string.gsub(name,'سعادتك','قناعتك نص __')
name = string.gsub(name,'القرد','__بعين امه غزال')
name = string.gsub(name,'عماها','يب يكحلها ___')
...
return '️┇اول من يكتب هذه الـSmile يفوز ››    `'..(redis:get(max..':Set_Smile:'..msg.chat_id_))..'`'
end
end
...
return 'عذرا ليس لديك النقاط بهذا العدد لبيعهم' 
end
if points == "0" then
...
الالعاب لبوت []]..redis:get(max..':NameBot:')..[[]  .
  ...
  ⋆ ⋆ ⋆ ⋆  ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆
  ⋆ امثله ⇽ لعبة امثله قديمه
  ⋆ معاني ⇽ لعبة معاني السمايلات 
  ⋆ اسئله ⇽ لعبة اسئله عامه
  ⋆ روليت ⇽ لعبة الروليت الشهيره
  ⋆ حزوره ⇽ لعبة الحزورات
  ⋆ ترتيب ⇽ لعبة ترتيب الكلمات
  ⋆ العكس ⇽ لعبة عكس الكلمه
  ⋆ تخمين ⇽ لعبة تخمين الكلمه 
  ⋆ الاسرع ⇽ لعبة اسرع واحد 
  ⋆ محيبس ⇽ لعبة محيبس الشهيره
  ⋆ المختلف ⇽ لعبة اختلاف السمايل
⋆ ⋆ ⋆ ⋆  ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆ ⋆
  ⋆ للمزيد من المعلومات 
...
return sendMsg(msg.chat_id_,msg.id_,'*  ياوحشش اجابتك صح    \n*   \n')
end
...
sendMsg(msg.chat_id_,msg.id_,"\nتم يحبيبي اضافة له { "..msg.text.." }* رساله")
...
sendMsg(msg.chat_id_,msg.id_,"\nتم يحبيبي اضافة له { "..msg.text.." }* نقطه .")
...
sendMsg(msg.chat_id_,msg.id_,'*مبروك فزت وطلعت المحيبس بالايد رقم { '..NUM..' }\nلقد حصلت على { 3 }من نقاط يمكنك استبدالهم برسائل *')   
...
sendMsg(msg.chat_id_,msg.id_,'اجاباتك صح ، اهنيك وربك 🤍')     
redis:incrby(max..':User_Points:'..msg.chat_id_..msg.sender_user_id_,1)  
...
sendMsg(msg.chat_id_,msg.id_,"اسف لا يمكنك تخمين عدد اكبر من ال { 20 } خمن رقم ما بين ال{ 1 و 20 } *\n")
return false  end 
...
redis:del(max..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
redis:del(max.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
sendMsg(msg.chat_id_,msg.id_,'\n*اح لقد خسرت في اللعبه \nحظ آوفر ي الخوي \n🚫╽كان الرقم الذي تم تخمينه { '..GETNUM..' }\n*')
else
sendMsg(msg.chat_id_,msg.id_,'تخمينك غلط \nارسل رقم تخمنه مره اخره \n*')
end
end
end
end
